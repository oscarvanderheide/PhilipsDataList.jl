...
This function only really _reads_ in the data but does not sort it into, for example,
k-spaces.
...
# Returns
- `samples_per_type::NamedTuple`: A NamedTuple with each field being an array of samples
  corresponding to one of the types of "complex data vectors" (see
  `COMPLEX_DATA_VECTOR_TYPES`).
- `attributes_per_type::NamedTuple`: A NamedTuple with each field being a DataFrame
  containing attributes corresponding to one of the types of "complex data vectors" (see
  `COMPLEX_DATA_VECTOR_TYPES`).
...
First, read a .list file into a vector of strings, each representing a line in the .list
file.
...
1. "general information"
    ...
    Examples of general are the maximum kx, ky and kz indices, oversampling factors and the
    number of channels.
...
2. "attributes" of the "complex data vectors" in the corresponding .data file.
    ...
    For example, each "complex data vector" could be a readout and the attributes tell which
    channel and ky index the readout corresponds to, how many bytes of data it contains and
    at what offset in the .data file it starts.

    The attributes are found in lines that do *not* start with "#" or ".".

# Returns
- `attributes::DataFrame`: A DataFrame containing the attributes of the "complex data
